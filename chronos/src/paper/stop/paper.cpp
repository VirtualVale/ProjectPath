...
int main(int argc, char** argv){
  ...
  list_goals[1] = {3, 0.7, -1, 2};
  list_goals[2] = {1, 1, 0, 0};
  list_goals[3] = {1, 1, 0, 0};
  list_goals[4] = {1, 1, 0, 0};
  ...
  for(int i=0; i<=4; i++){
    ...
    if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
      ROS_INFO("Robot 1: Postition %d reached!", i);
    else
  ...
  ROS_ERROR("Robot 1: stop-strategy time_sum %f", time_sum.toSec());
