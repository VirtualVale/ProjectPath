...
int main(int argc, char** argv){
  ...
  list_goals[0] = {1.3, 1.3, 0, 17};
  list_goals[1] = {6, 0.5, -1, 0};
  list_goals[2] = {6, 0.5, -1, 0};
  list_goals[3] = {6, 0.5, -1, 0};
  list_goals[4] = {6, 0.5, -1, 0};
  ...
  for(int i=0; i<=4; i++){
    ...
    if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
      ROS_INFO("Robot 2: Postition %d reached!", i);
    else
  ...
  ROS_ERROR("Robot 2: stop-strategy time_sum %f", time_sum.toSec());
